...
fn main() {
    ...
    // Dummy raw chunk (4096 zero bytes) — tính d_ji và chunk_limbs nhất quán
    // để constraint fold(limbs)==d_ji luôn thỏa mãn trong test.
    let dummy_chunk = vec![0u8; 4096];
    let d_ji = bytes_to_fr(&dummy_chunk);
    let chunk_limbs = chunk_to_limbs(&dummy_chunk);
    ...
    // Root — phải tôn trọng thứ tự trái/phải theo bit của (j_i - 1),
    // giống hệt conditional_swap_gadget trong circuit:
    //   is_right = 1 → current nằm BÊN PHẢI: hash(sibling, current)
    //   is_right = 0 → current nằm BÊN TRÁI: hash(current, sibling)
    let mut current_hash = hash_2(r_ji, s_ji);
    for bit in 0..18 {
        let is_right = (j_i_minus_1 >> bit) & 1 == 1;
        current_hash = if is_right {
            hash_2(Fr::ZERO, current_hash)
        } else {
            hash_2(current_hash, Fr::ZERO)
        };
    }
