...
use core_primitives::chunking::num_limbs;
...
use nova_snark::{
    ...
    provider::{
        hyperkzg::EvaluationEngine as HyperKZGEngine,
        ipa_pc::EvaluationEngine as IpaEngine,
...
// Primary curve: BN254 with HyperKZG commitment (O(log n) verifier, EVM-friendly)
pub type G1 = Bn256EngineKZG;
// Secondary curve: Grumpkin with IPA (cycle partner of BN254)
pub type G2 = GrumpkinEngine;

pub type NovaFr = <G1 as Engine>::Scalar;

pub type SpartanPrimary   = nova_snark::spartan::ppsnark::RelaxedR1CSSNARK<G1, HyperKZGEngine<G1>>;
pub type SpartanSecondary = nova_snark::spartan::ppsnark::RelaxedR1CSSNARK<G2, IpaEngine<G2>>;
...
/// witness: chunk_limbs (raw chunk — possession), S_ji-1, S_ji, Merkle path
///
/// D_ji KHÔNG còn là witness: nó được TÍNH trong circuit = fold(chunk_limbs),
/// nên host không thể cung cấp D_ji lệch với dữ liệu thật.
#[derive(Clone, Debug)]
pub struct EngramStepCircuit {
    ...
    /// Chunk possession witness: các limb 31-byte của raw chunk tại j_i
    /// (`num_limbs(chunk_size_bytes)` phần tử — 133 với chunk 4KB).
    ///
    /// Mục đích: ngăn node xóa 100% dữ liệu client mà vẫn pass proof bằng cách chỉ
    /// giữ digest 32 byte. Circuit tính D_ji = fold(chunk_limbs) rồi đưa thẳng vào
    /// R_ji, nên prover PHẢI biết preimage thật → phải giữ raw data.
    pub chunk_limbs: Vec<CoreFr>,
    /// Kích thước chunk (bytes). Ép acc khởi tạo của fold; phải khớp
    /// `EngramConfig::chunk_size_bytes` đã dùng lúc sealing.
    pub chunk_size_bytes: usize,
}

/// Helper: chuyen CoreFr sang NovaFr.
/// Sau khi doi sang BN254: CoreFr = BN254 Scalar = NovaFr, ca hai dung cung type.
/// Van dung repr de tranh unsafe transmute.
...
impl StepCircuit<NovaFr> for EngramStepCircuit {
    ...
    fn synthesize<CS: ConstraintSystem<NovaFr>>(
        ...
        // --- 5b. CHUNK POSSESSION PROOF ---
        // Tính D_ji NGAY TRONG CIRCUIT từ raw chunk, replicate chính xác
        // core_primitives::chunking::fold_limbs():
        //     acc_0 = Fr(chunk_size_bytes)   (ép hằng số)
        //     acc_{k+1} = hash_2(acc_k, limb_k)
        //     D_ji = acc_last
        //
        // BẢO MẬT: prover chỉ giữ digest 32 byte KHÔNG thể dựng được limbs hợp lệ
        // (preimage resistance) → buộc phải giữ raw data. Nếu limbs sai, D_ji sai
        // → R_ji sai → Merkle root != sealed_root → verify reject.
        let d_ji_var = {
            // Lá chắn 1: prover trung thực nhưng cấu hình sai (vd đổi chunk_size mà
            // quên đổi limbs) dừng NGAY với lỗi rõ ràng, thay vì "UnSat" khó truy.
            // Trả Err thay vì assert!: profile release đặt panic="abort" nên panic sẽ
            // giết tiến trình; SynthesisError được Nova truyền lên thành Result.
            //
            // Lá chắn 2 (chống prover ác ý gỡ check này): Nova cố định shape từ lúc
            // setup → sai số limb làm lệch shape → verify từ chối. Đã kiểm chứng
            // bằng bin `attack_chunk`.
            let expected_limbs = num_limbs(self.chunk_size_bytes);
            if self.chunk_limbs.len() != expected_limbs {
                return Err(SynthesisError::Unsatisfiable(format!(
                    "chunk_limbs có {} limb, nhưng chunk {} byte cần đúng {} limb",
                    self.chunk_limbs.len(),
                    self.chunk_size_bytes,
                    expected_limbs
                )));
            }

            let acc_init = NovaFr::from(self.chunk_size_bytes as u64);
            let mut acc_chunk =
                AllocatedNum::alloc(cs.namespace(|| "chunk_acc_init"), || Ok(acc_init))?;
            // Ép acc_0 = Fr(chunk_size_bytes) — prover không thể chọn giá trị khác
            cs.enforce(
                || "enforce_chunk_acc_init",
                |lc| lc + acc_chunk.get_variable(),
                |lc| lc + CS::one(),
                |lc| lc + (acc_init, CS::one()),
            );

            for (k, limb_val) in self.chunk_limbs.iter().enumerate() {
                let limb_var = AllocatedNum::alloc(
                    cs.namespace(|| format!("chunk_limb_{}", k)),
                    || Ok(core_fr_to_nova_fr(*limb_val)),
                )?;
                acc_chunk = hash_2_gadget(
                    cs.namespace(|| format!("chunk_fold_{}", k)),
                    &acc_chunk,
                    &limb_var,
                )?;
            }
            acc_chunk // == D_ji, dùng trực tiếp cho R_ji bên dưới
        };
