...
`seal()` phía Python nay hiện thực **đúng Thuật toán 1c** mà mạch kiểm:

    D_i = fold limb chunk
    R_i = H4(D_i, S_{i-1}, i, replica_id)
    S_i = H2(S_{i-1}, R_i)

Có test chốt bằng cách tính tay theo công thức Rust: `test_seal_khop_mach.py`.

Bản SeqWide có fan-in giữ lại dưới tên `seal_seqwide()`, **không nằm trên đường
chạy chính**, để phần phân tích bao đóng không mất.

**Hai thứ MẤT khi bỏ fan-in, phải ghi vào bài:**

① **Điểm mốc lại hoạt động.** Kẻ gian lưu S mỗi 1.000 chunk, hết 268 KB, rồi
nối chuỗi từ mắt gần nhất trong **0,04 giây**.

② **Phần hấp thụ chunk tách khỏi chuỗi.** `D_i` chỉ phụ thuộc chunk i, nên
**97,8 %** công việc song song hoá được; nút thật 12 nhân mất 23 phút còn kẻ
gian song song hoá mất 4,92 phút.

Nghĩa là **mọi con số `t_regen` suy từ giả định có fan-in KHÔNG áp dụng** cho
mạch hiện tại. Section 4 phải viết lại theo 1c, hoặc co lại theo P1.14 của thầy.
